...
watch(activeSelectionType, (newType) => {
    if (newType !== 'none') {
        isPropertySheetOpen.value = true;
    }
});
...
const addNodeToCenter = (nodeType, shape, emojiOrUrl = null) => {
    const flowWrapper = document.querySelector('.vue-flow');
    if (!flowWrapper) return;
    ...
    const newNode = {
        ...
        position: projectedPosition,
        data: { 
            label: shape === 'emoji' || shape === 'image' ? '' : (shape === 'text' ? 'Judul Teks' : (shape === 'paragraph' ? 'Tulis paragraf panjang atau keterangan di sini...' : 'New Node')), 
            shape: shape,
            emoji: emojiOrUrl,
            imageUrl: emojiOrUrl,
            bgColor: shape === 'text' || shape === 'paragraph' || shape === 'group' ? 'transparent' : '#ffffff',
            textColor: '#111827',
            fontSize: shape === 'text' ? 24 : 14,
            fontFamily: 'Inter',
            isBorderOnly: shape === 'group' ? true : false,
            width: shape === 'group' ? 300 : (shape === 'paragraph' ? 250 : null),
            height: shape === 'group' ? 200 : null,
            isNew: true,
            onAddChild: handleAddChild,
            onAddSibling: handleAddSibling
        }
    ...
    addNodes([newNode]);
    commitHistory();
    isShapeSheetOpen.value = false; // Close the sheet after adding
};
...
<template>
        <div class="flex flex-col w-full bg-gray-200 h-[100dvh] relative">
            ...
            <div v-if="isMobileMenuOpen" class="absolute top-[50px] right-2 w-56 bg-white shadow-xl rounded-lg border border-gray-200 z-[100] flex flex-col py-2">
                ...
                <button @click="exportToSvg(); isMobileMenuOpen = false" class="px-4 py-2 text-sm text-left hover:bg-gray-100 w-full flex items-center">
                ...
                <button v-if="props.canEdit" @click="isShareModalOpen = true; isMobileMenuOpen = false" class="px-4 py-2 text-sm text-left hover:bg-gray-100 w-full flex items-center">
            ...
            <div v-if="props.canEdit && settings.diagramMode !== 'mindmap' && isShapeSheetOpen" class="absolute bottom-16 left-2 right-2 bg-white rounded-t-xl shadow-[0_-10px_40px_rgba(0,0,0,0.15)] flex flex-col z-50 border-t border-gray-200 max-h-[60vh] overflow-hidden pb-4">
                <div class="flex justify-between items-center p-3 border-b bg-gray-50">
                    <span class="font-semibold text-gray-700 text-sm">Tambahkan Shape</span>
                    <button @click="isShapeSheetOpen = false" class="p-1 rounded bg-gray-200 text-gray-600">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                    </button>
                </div>
                
                <div class="overflow-y-auto p-4 flex flex-wrap gap-4 justify-center">
                    <!-- Text -->
                    <button @click="addNodeToCenter('custom', 'text')" class="text-xl font-bold font-serif text-gray-700 bg-gray-100 hover:bg-gray-200 w-16 h-16 rounded-xl flex items-center justify-center shadow-sm">T</button>
                    <!-- Paragraph -->
                    <button @click="addNodeToCenter('custom', 'paragraph')" class="text-sm font-serif text-gray-700 bg-gray-100 hover:bg-gray-200 w-16 h-16 rounded-xl flex items-center justify-center shadow-sm">P</button>
                    <!-- Box -->
                    <button @click="addNodeToCenter('custom', 'box')" class="w-16 h-12 border-2 border-gray-400 bg-white rounded-md shadow-sm"></button>
                    <!-- Pill -->
                    <button @click="addNodeToCenter('custom', 'pill')" class="w-16 h-10 border-2 border-gray-400 bg-white rounded-full shadow-sm"></button>
                    <!-- Diamond -->
                    <button @click="addNodeToCenter('custom', 'diamond')" class="w-16 h-16 shadow-sm">
                        <svg class="w-full h-full pointer-events-none" viewBox="0 0 100 100" preserveAspectRatio="none"><polygon points="50,0 100,50 50,100 0,50" fill="white" stroke="#9ca3af" stroke-width="3" vector-effect="non-scaling-stroke" /></svg>
                    </button>
                    <!-- Parallelogram -->
                    <button @click="addNodeToCenter('custom', 'parallelogram')" class="w-16 h-12 shadow-sm">
                        <svg class="w-full h-full pointer-events-none" viewBox="0 0 100 100" preserveAspectRatio="none"><polygon points="15,0 100,0 85,100 0,100" fill="white" stroke="#9ca3af" stroke-width="3" vector-effect="non-scaling-stroke" /></svg>
                    </button>
                    <!-- Hexagon -->
                    <button @click="addNodeToCenter('custom', 'hexagon')" class="w-16 h-12 shadow-sm">
                        <svg class="w-full h-full pointer-events-none" viewBox="0 0 100 100" preserveAspectRatio="none"><polygon points="25,0 75,0 100,50 75,100 25,100 0,50" fill="white" stroke="#9ca3af" stroke-width="3" vector-effect="non-scaling-stroke" /></svg>
                    </button>
                    <!-- Cylinder -->
                    <button @click="addNodeToCenter('custom', 'cylinder')" class="w-12 h-14 border-2 border-gray-400 bg-white shadow-sm" style="border-radius: 50% / 15%;"></button>
                    <!-- Emoji -->
                    <button @click="addNodeToCenter('custom', 'emoji', '😀')" class="text-3xl flex items-center justify-center w-16 h-16 bg-gray-50 border border-gray-200 rounded-xl shadow-sm" v-html="defaultEmojiIcon"></button>
                    
                    <!-- Group / Area -->
                    <button @click="addNodeToCenter('custom', 'group')" class="w-16 h-16 border-2 border-gray-400 border-dashed bg-gray-50 rounded-xl flex items-center justify-center text-xs text-gray-400 shadow-sm">Area</button>
                    
                    <!-- Image Upload -->
                    <div class="relative w-16 h-16 border-2 border-gray-400 border-dashed bg-white rounded-xl shadow-sm flex items-center justify-center text-gray-400">
            ...
            <!-- PROPERTIES MOBILE DRAWER -->
            <div v-if="isPropertySheetOpen" class="absolute bottom-0 left-0 right-0 bg-white rounded-t-xl shadow-[0_-10px_40px_rgba(0,0,0,0.15)] flex flex-col z-50 border-t border-gray-200 h-[65vh] overflow-hidden">
                <!-- PANEL HEADER -->
                <div class="h-14 border-b bg-gray-50 flex items-center justify-between px-4 shrink-0 shadow-sm">
                    ...
                    <button @click="isPropertySheetOpen = false" class="p-1 rounded bg-gray-200 text-gray-600 hover:bg-gray-300">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                    </button>
                </div>
